package com.actas.cmob.DTO.Themoon;

import lombok.Getter;
import lombok.Setter;

@Getter
@Setter
public class CA609Dto {

    private String dbnm;
    //CA609
    private String custcd;
    private String spjangcd;
    private String baldate;
    private String balnum;
    private String balseq;
    private String pcode;
    private String moncls;
    private String monrate;
    private String qty;
    private String uamt;
    private String samt;
    private String tamt;
    private String mamt;
    private String remark;
    private String ibgflag;
    private String qcflag;
    private int qcqty;
    private String ibgqty;
    private String pumdate;
    private String pumnum;
    private String pumseq;
    private String pumdv;
    private String deldate;
    private String taxflag;
    private String projno;
    private String indate;
    private String inperid;
    private String actcd;
    private String divicd;
    private String psize;
    private String punit;
    private String actnm;
    private String orddate;
    private String ordnum;
    private String ordseq;
    private String compflag;
    private String buddate;
    private String budnum;
    private String budseq;
    private String budflag;
    private String size;
    private String code88;

    // CA608
    private String baldv;
    private String balno;
    private String pumdivicd;
    private String taxcls;
    private String setcls;
    private String store;
    private String domcls;
    private String sunflag;
    private String sunamt;
    private String ischdate;
    private String bigo;
    private String maildv;
    private String cdflag;
    private String taxgubun;
    private String ibgdate;
    private String ibgnum;
    private String compperid;
    private String compdivicd;
    private String compdate;
    private String state;
    private String hagigan;
    private String sogigan;
    private String balpernm;
    private String pumperid;
    private String title;

    // Lot
    private String cltcd;
    private String delnum;
    private String delseq;
    private String pname;
    private String coilqty;

    // CA501(B)
    private String lotno;
    private String qcdate;
    private String qcnum;

    private String userid;
    private String winid;
    private String ipaddr;
    private String usernm;
    private String winnm;

    private String wmqty;
    private String cltnm;

    private int ll_cnt;
    private int ll_cnt1;
    private int ll_cnt2;
    private int ll_cnt3;
    private int ll_cnt4;

    private String baldateList;
    private String balnumList;
    private String balseqList;
    private String wqtyList;
    private String qcdateList;
    private String qcnumList;
    private String wqty;
    private String buton;
    private String comcd;
    private String qcdv;

    private int balqty;


}
